...
export const convertParagraphItem = (paragraphItem: ParagraphItem): string => {
  ...
  paragraphItem.content.forEach((contentItem) => {
    if (typeof contentItem === 'string') {
      paragraphString += contentItem.replace(/\<\w*\>/g, "").replace(/\<\/\w*\>/g, "");
    } else if (contentItem.type.includes('Text') && contentItem.type !== 'Picture') {
...
export const convertTitleCardItem = (titleCardItem: TitleCardItem | SubtitleCardItem): string => {
...
export const articleToNarration = (articleJson: ArticleJson): string => {
  ...
  articleJson.forEach((articleItem) => {
    switch (articleItem.type) {
      ...
      case 'SubtitleCard':
        narrationString += convertTitleCardItem(articleItem as SubtitleCardItem);
        break;
